...
static PyMethodDef ClientMethods[] = {
    {"open",  client_open, METH_VARARGS, "Opens port. Usage client.open(port_name)"},
    {"close",  client_close, METH_NOARGS, "Closes port. Usage client.close()"},

    {"gyro_count", client_gyro_count, METH_NOARGS, "Returns count of collected gyro data"},
    {"accel_count", client_accel_count, METH_NOARGS, "Returns count of collected accel data"},

    {"get_gyro",  client_get_gyro, METH_NOARGS, "Returns list of all collected gyro data "},
    {"get_accel",  client_get_accel, METH_NOARGS, "Returns list of all collected accel data "},
    {NULL,NULL,0,NULL}/* Sentinel */
};

PyMODINIT_FUNC initforce_client(void)
{
	PyObject* m;
	m =Py_InitModule("force_client",ClientMethods);
	if(!m)
		return;
